...
function displayResumeData(data) {
    ...
    return `
      <div class="accordion-item rounded-3 shadow-sm mb-3 border-0">
        <h2 class="accordion-header" id="heading${id}">
          <button class="accordion-button ${show ? '' : 'collapsed'} fw-semibold" 
  ...
  if (data.personal_info) {
    ...
    let content = `
      ${info.name ? `<p><i class="fas fa-user me-2 text-primary"></i><strong>Name:</strong> ${info.name}</p>` : ''}
      ${info.email ? `<p><i class="fas fa-envelope me-2 text-danger"></i><strong>Email:</strong> ${info.email}</p>` : ''}
      ${info.phone ? `<p><i class="fas fa-phone me-2 text-success"></i><strong>Phone:</strong> ${info.phone}</p>` : ''}
    `;
    ...
    outputAccordion.insertAdjacentHTML('beforeend', createAccordionItem(`Personal${sectionCounter++}`, '<i class="fas fa-id-card me-2"></i>Personal Information', content, true));
  }
...
if (data.experience && data.experience.length > 0) {
  ...
  if (data.skills && data.skills.length > 0) {
    ...
    outputAccordion.insertAdjacentHTML('beforeend', createAccordionItem(`Skills${sectionCounter++}`, '<i class="fas fa-tools me-2"></i>Skills', content));
  }
  ...
  if (data.certifications && data.certifications.length > 0) {
    ...
    outputAccordion.insertAdjacentHTML('beforeend', createAccordionItem(`Certifications${sectionCounter++}`, '<i class="fas fa-award me-2"></i>Certifications', content));
  }
  ...
  if (data.awards && data.awards.length > 0) {
    ...
    outputAccordion.insertAdjacentHTML('beforeend', createAccordionItem(`Awards${sectionCounter++}`, '<i class="fas fa-medal me-2"></i>Awards', content));
  }
